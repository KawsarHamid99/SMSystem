...
def grade():
    ...
    Marks.objects.filter(subject_marks__gte=70,subject_marks__lt=80).update(subject_grade="A")
    Marks.objects.filter(subject_marks__gte=65,subject_marks__lt=70).update(subject_grade="A-")
    Marks.objects.filter(subject_marks__gte=50,subject_marks__lt=65).update(subject_grade="B")
    Marks.objects.filter(subject_marks__gte=40,subject_marks__lt=50).update(subject_grade="C")
    Marks.objects.filter(subject_marks__gte=33,subject_marks__lt=40).update(subject_grade="D")
    Marks.objects.filter(subject_marks__lte=33).update(subject_grade="F")

class ResultClass(View):
...
class EditClass(View):
    ...
    def post(self,request, id,roll):
        ...
        if form.is_valid():
            ...
            return render(request, 'result/updateResult.html', {'form': form,'success': True,"students":student})
